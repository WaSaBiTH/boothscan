...
// Note: SplitText is a premium GSAP plugin, but assuming user has access or it works in their env.
// If it fails, they will need the gsap-trial or a valid club greensock membership.
// Since they provided the snippet, we will implement it verbatim as requested.
    ...
    useEffect(() => {
        ...
        const build = () => {
            ...
            chars.forEach(ch => {
                ...
                for (const _ of Array.from({ length: rolls })) {
                ...
                if (colorFrom) {
                    ; (inner.style as any).color = colorFrom
                }
        ...
        const randomizeScrambles = () => {
            ...
            wrappersRef.current.forEach(w => {
                ...
                for (const kid of kids.slice(1, -1)) {
        ...
        const play = () => {
            ...
            const tl = gsap.timeline({
                ...
                onRepeat: () => {
                    ...
                    gsap.set(strips, {
                        x: (_i, t: HTMLElement) => Number.parseFloat(t.getAttribute("data-start-x") || "0"),
                    })
                    onShuffleComplete?.()
                },
                onComplete: () => {
            ...
            const addTween = (targets: HTMLElement[], at: number) => {
                tl.to(
                    targets,
                    {
                        x: (_i, t: HTMLElement) => Number.parseFloat(t.getAttribute("data-final-x") || "0"),
                        duration,
                        ease,
                        force3D: true,
